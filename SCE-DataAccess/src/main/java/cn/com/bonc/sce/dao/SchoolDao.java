...
public interface SchoolDao extends JpaRepository< School, Integer > {
    ...
    @Query(value = " select * FROM \n" +
            " (SELECT USER_ID,LOGIN_NAME,LOGIN_PERMISSION_STATUS,IS_FIRST_LOGIN,ORGANIZATION_ID,CREATE_TIME FROM  STARCLOUDPORTAL.SCE_COMMON_USER  WHERE user_type= 3 AND IS_DELETE = 1 AND  ORGANIZATION_ID  IS NOT NULL) \n" +
            "  u LEFT  JOIN \n" +
